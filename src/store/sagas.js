...
function* api_v1_events_listWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_list, action)
    yield put(actions.api_v1_events_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_listFailed(err, action))
  }
}
function* api_v1_events_listWatcher() {
  yield takeEvery(types.API_V1_EVENTS_LIST, api_v1_events_listWorker)
}
function* api_v1_events_createWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_create, action)
    yield put(actions.api_v1_events_createSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_createFailed(err, action))
  }
}
function* api_v1_events_createWatcher() {
  yield takeEvery(types.API_V1_EVENTS_CREATE, api_v1_events_createWorker)
}
function* api_v1_events_readWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_read, action)
    yield put(actions.api_v1_events_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_readFailed(err, action))
  }
}
function* api_v1_events_readWatcher() {
  yield takeEvery(types.API_V1_EVENTS_READ, api_v1_events_readWorker)
}
function* api_v1_events_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_update, action)
    yield put(actions.api_v1_events_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_updateFailed(err, action))
  }
}
function* api_v1_events_updateWatcher() {
  yield takeEvery(types.API_V1_EVENTS_UPDATE, api_v1_events_updateWorker)
}
function* api_v1_events_partial_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_partial_update, action)
    yield put(actions.api_v1_events_partial_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_partial_updateFailed(err, action))
  }
}
function* api_v1_events_partial_updateWatcher() {
  yield takeEvery(
    types.API_V1_EVENTS_PARTIAL_UPDATE,
    api_v1_events_partial_updateWorker
  )
}
function* api_v1_events_deleteWorker(action) {
  try {
    const result = yield call(apiService.api_v1_events_delete, action)
    yield put(actions.api_v1_events_deleteSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_events_deleteFailed(err, action))
  }
}
function* api_v1_events_deleteWatcher() {
  yield takeEvery(types.API_V1_EVENTS_DELETE, api_v1_events_deleteWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    ...
    api_v1_events_listWatcher,
    api_v1_events_createWatcher,
    api_v1_events_readWatcher,
    api_v1_events_updateWatcher,
    api_v1_events_partial_updateWatcher,
    api_v1_events_deleteWatcher,
